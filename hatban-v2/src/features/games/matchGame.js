...
export function mountMatch(root,done,{count=20}={}) {
  ...
  board.style.setProperty('--card-columns',count<=12?5:count<=24?6:count<=40?8:count<=56?10:12);board.style.setProperty('--mobile-columns',count<=20?4:count<=40?5:6);root.append(info,board);
