import { checkTicTacToe, createMathQuestion, createPairs, tenSecondError } from './gameLogic.js';
...
export function mountGame(id,onBack) {
 ...
 const done=(score,record={})=>{const entry=records.add(id,score,record);const p=document.createElement('p');p.className='game-completion-notice';p.textContent='완료! 이 기기 기록에 저장했어요.';root.append(p);return entry;};
 ...
 else if(id==='baseball') cleanup=mountBaseball(root,done);
 else if(id==='math'){let score,count,q,end,timerId,endsAt;const text=document.createElement('p'),time=document.createElement('p'),input=document.createElement('input'),go=document.createElement('button'),again=document.createElement('button');go.textContent='확인';again.textContent='다시 하기';function finish(){if(end)return;end=true;clearInterval(timerId);text.textContent='최종 점수 '+score+'점';done(score,{questions:count});go.disabled=true;}function reset(){clearInterval(timerId);score=0;count=0;end=false;q=createMathQuestion();input.value='';go.disabled=false;endsAt=Date.now()+10000;timerId=setInterval(()=>{const remain=Math.max(0,endsAt-Date.now());time.textContent='남은 시간 '+Math.ceil(remain/1000)+'초 · 점수 '+score+'점';if(!remain)finish();},100);text.textContent=q.text;time.textContent='남은 시간 10초 · 점수 0점';}go.onclick=()=>{if(end)return;if(Number(input.value)===q.answer)score++;count++;q=createMathQuestion();text.textContent=q.text;input.value='';};again.onclick=reset;reset();root.append(time,text,input,go,again);cleanup=()=>clearInterval(timerId);}
